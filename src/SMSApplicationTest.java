...
class ProcessorTest implements ConnectionObserver{
	String smscHost = "";
	int smscPort = 5019;
	String smscSystemID = "";
	String smscPassword = "";
	...
	public ProcessorTest()
	{
    	...
    	dbConnection2 = new MSConnection(logger, properties);
...
//					initSubmitSMTimer();
			...
			case SMPPPacket.SUBMIT_SM_RESP:				
				...
				try
				{
					...
					{
						status = -1;
						logger.writeReceived("SUBMIT_SM_RESP error|Errorcode: " + submitSMResp.getCommandStatus()+ "|Message Id: " + submitSMResp.getSequenceNum());
					}
					else
					{
						status = 1;	
						logger.writeReceived("SUBMIT_SM_RESP Success" +  "|Message Id: " + submitSMResp.getSequenceNum());
					}
				}
				catch(Exception ex)
				{
				...
				if(dbConnection2.checkConnection())
				{
					dbConnection2.updateOnReceive(submitSMResp, status,smsResponse);					
				}
				break;
			...
			while(rs.next())
			{
				...
				{	
					...
					else 	//some exception occurs
					{
						...
						writeMessage("Exception in setting SMS info|Id: " + smsDTO.Id);
				...
				catch(Exception ex)
				{
					...
					writeMessage("Exception in loop for each message: " + ex.getMessage() + " | Id: " + smsDTO.Id);
				}
			}
...
//			  				else	//Other types e.g. SMS, SMS Campaign, Schedule.
		  		...
		  		catch(Exception ex)
		  		{
		  			...
		  			writeMessage("Exception for each message sending: " + ex.getMessage() + " | Id: " + smsDTO.Id);
...
public class SMSApplicationTest
{
	...
	{
		ProcessorTest processor = new ProcessorTest();
